...
namespace {
struct LowerContractsPassInFunctions
    : PassWrapper<LowerContractsPassInFunctions, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(LowerContractsPassInFunctions);
  StringRef getArgument() const override { return "lower-contracts"; }
  StringRef getDescription() const override {
    return "Convert verif ops in functions";
  }
...
LogicalResult runOnFuncs(FunctionOpInterface funcOp, ModuleOp mlirModule) {
  OpBuilder mlirModuleBuilder(mlirModule);
  mlirModuleBuilder.setInsertionPointAfter(funcOp);
  OpBuilder funcOpBuilder(funcOp);

  // Collect contract ops
  SmallVector<ContractOp> contracts;
  funcOp.walk([&](ContractOp op) { contracts.push_back(op); });
  ...
  for (unsigned i = 0; i < contracts.size(); i++) {
    ...
    auto name = mlirModuleBuilder.getStringAttr(
        funcOp.getNameAttr().getValue() + "_CheckContract_" + Twine(i));
  ...
  for (auto contract : contracts) {
    ...
    funcOpBuilder.setInsertionPointAfter(contract);
    if (failed(inlineContract(contract, funcOpBuilder, true)))
      return failure();
    contract.erase();
  }
...
void LowerContractsPassInFunctions::runOnOperation() {
  auto mlirModule = getOperation();
  for (auto module : mlirModule.getOps<FunctionOpInterface>())
    if (failed(runOnFuncs(module, mlirModule)))
      signalPassFailure();
}

namespace mlir::triton::AMD {
void registerLowerContractsPassInFunctions() {
  PassRegistration<LowerContractsPassInFunctions>();
}
} // namespace mlir::triton::AMD
